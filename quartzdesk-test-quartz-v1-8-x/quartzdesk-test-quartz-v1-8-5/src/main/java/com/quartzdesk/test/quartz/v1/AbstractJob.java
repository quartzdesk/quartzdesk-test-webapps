...
import com.quartzdesk.test.common.CommonUtils;
...
/**
 * Common base class for all jobs invoked by the Spring Quartz scheduler. In the Spring framework, the scheduler
 * running
 * jobs extending this class should be configured to use the {@link org.springframework.scheduling.quartz.SpringBeanJobFactory}.
 *
    ...
    implements Job
{
  ...
  /**
   * Key of the scheduler context property that holds the {@link ApplicationContext} instance. Please check refer to
   * the
   * {@code applicationContextSchedulerContextKey} property of the {@link SchedulerFactoryBean}.
   */
  private static final String SCHEDULER_CONTEXT_APPLICATION_CONTEXT_KEY = "applicationContext";

  /**
   * The method invoked by the Spring scheduler. This method simply delegates the execution to the {@link
   * #executeJob(JobExecutionContext)} method.
   *
  ...
  /**
   * Returns true if this job should be interrupted, false otherwise. Interruptible jobs (implementing the {@link
   * InterruptableJob} Quartz interface) should override this method. <p> This method in this base class always returns
   * false. </p>
   *
